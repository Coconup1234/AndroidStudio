package com.example.exam_shop2022;

import androidx.appcompat.app.AppCompatActivity;

import android.content.Intent;
import android.os.Bundle;
import android.view.View;
import android.widget.Button;
import android.widget.CheckBox;
import android.widget.EditText;
import android.widget.ImageButton;
import android.widget.ImageView;
import android.widget.RadioButton;
import android.widget.TextView;
import android.widget.Toast;


public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ImageView img_product_main;
    EditText edit_count;
    TextView txt_price, txt_delivery, txt_pay;
    CheckBox chk_agree;
    RadioButton radio1, radio2, radio3 ,radio4;
    Button btn_pay;
    ImageButton btn_minus, btn_plus;
    int val_price;                  // 제품의 총 가격(배송비 미포함)
    int val_delivery;               // 배송비 ( 제품 총 가격이 10000원 이상이면 0원, 미만이면 2500원)
    int val_pay;                    // 총 결제 금액
    int selected_product = 2000;      // 선택한 제품 한개의 가격
    int selected_count;
    String selected_name = "";
    // 선택한 수량

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);


        img_product_main = findViewById(R.id.img_product_main);
        radio1 = findViewById(R.id.radio1);
        radio2 = findViewById(R.id.radio2);
        radio3 = findViewById(R.id.radio3);
        radio4 = findViewById(R.id.radio4);
        btn_pay = findViewById(R.id.btn_pay);
        txt_price = findViewById(R.id.txt_price);
        txt_delivery = findViewById(R.id.txt_delivery);
        txt_pay = findViewById(R.id.txt_pay);
        chk_agree = findViewById(R.id.chk_agree);
        edit_count = findViewById(R.id.edit_count);
        btn_minus= findViewById(R.id.btn_minus);
        btn_pay= findViewById(R.id.btn_pay);
        btn_plus= findViewById(R.id.btn_plus);

        radio1.setOnClickListener(this);
        radio2.setOnClickListener(this);
        radio3.setOnClickListener(this);
        radio4.setOnClickListener(this);

        btn_minus.setOnClickListener(this);
        btn_pay.setOnClickListener(this);
        btn_plus.setOnClickListener(this);


    }

    private void sumTotal() {
        int selected_count = Integer.parseInt(edit_count.getText().toString());
        val_price = selected_product * selected_count;
        txt_price.setText(val_price + "원");

        if (val_price >= 10000) {
            txt_delivery.setText("1만원 구매 혜텍 적용");
            val_delivery = 0;

        } else {
            val_delivery = 2500;
            txt_delivery.setText(val_delivery + "원");
        }
        val_pay = val_price + val_delivery;
        txt_pay.setText(val_pay + "원");
    }


    @Override
    public void onClick(View v) {

        switch (v.getId()) {
            case R.id.radio1:
                radio1.setChecked(true); // 선택된 상태로 설정
                sumTotal();
                img_product_main.setImageResource(R.drawable.coffee1);
                selected_product = 2000;
                //selected_name = findViewById(R.id.radio1).getTag().toString();
                sumTotal();
                break;
            case R.id.radio2:
                radio2.setChecked(true); // 선택된 상태로 설정
                sumTotal();
                img_product_main.setImageResource(R.drawable.coffee2);
                selected_product = 3000;
                //selected_name = findViewById(R.id.radio2).getTag().toString();
                sumTotal();
                break;
            case R.id.radio3:
                radio3.setChecked(true); // 선택된 상태로 설정
                sumTotal();
                img_product_main.setImageResource(R.drawable.coffee3);
                selected_product = 4000;
                //selected_name = findViewById(R.id.radio3).getTag().toString();
                sumTotal();
                break;
            case R.id.radio4:
                radio4.setChecked(true); // 선택된 상태로 설정
                sumTotal();
                img_product_main.setImageResource(R.drawable.coffee4);
                selected_product = 5000;
                //selected_name = findViewById(R.id.radio4).getTag().toString();
                sumTotal();
                break;
            case R.id.btn_minus:
                selected_count = Integer.parseInt(edit_count.getText().toString());
                if (selected_count == 1) {
                    Toast.makeText(this, "최소 수량은 1 입니다", Toast.LENGTH_SHORT).show();
                } else {
                    --selected_count;
                    edit_count.setText(String.valueOf(selected_count));
                    sumTotal();
                }
                break;
            case R.id.btn_pay:
                if(chk_agree.isChecked()){
                    Intent intent = new Intent(MainActivity.this, SubActivity.class);
                    intent.putExtra("item_name",selected_name);
                    intent.putExtra("item_count",selected_count);
                    intent.putExtra("item_pay",val_pay);
                    startActivity(intent);
                }else{
                    Toast.makeText(this, "동의 해", Toast.LENGTH_SHORT).show();

                }break;
            case R.id.btn_plus:
                selected_count = Integer.parseInt(edit_count.getText().toString());
                if(selected_count == 5){
                    Toast.makeText(this, "최소 수량은 5 입니다", Toast.LENGTH_SHORT).show();
                }else{
                    ++selected_count;
                    edit_count.setText(String.valueOf(selected_count));
                    sumTotal();
                }
                break;
        }
    }
}